package zzh.lifeplayer.music.fragments.backup
...
class BackupViewModel : ViewModel() {
    ...
    fun loadBackups() {
        BackupHelper.getBackupRoot()
            .listFiles { _, name ->
                return@listFiles name.endsWith(BackupHelper.BACKUP_EXTENSION)
            }
            ?.toList()
            ?.let { backupsMutableLiveData.value = it }
    }
    ...
    suspend fun restoreBackup(
        ...
        if (
            contents.contains(BackupContent.SETTINGS) or
                contents.contains(BackupContent.CUSTOM_ARTIST_IMAGES)
        ) {
            // We have to restart App when Preferences i.e. Settings or Artist Images are to be
            // restored
