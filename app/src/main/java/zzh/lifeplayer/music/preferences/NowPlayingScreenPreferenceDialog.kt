...
class NowPlayingScreenPreference
@JvmOverloads
constructor(
    context: Context,
        ...
        val viewPager =
            view.findViewById<ViewPager>(R.id.now_playing_screen_view_pager)
                ?: throw IllegalStateException(
                    "Dialog view must contain a ViewPager with id 'now_playing_screen_view_pager'"
                )
...
private fun isNowPlayingThemes(screen: NowPlayingScreen): Boolean {
    //  return (screen == Full || screen == Card || screen == Plain || screen == Blur || screen ==
    // Color || screen == Simple || screen == BlurCard || screen == Circle || screen == Adaptive) &&
    // !App.isProVersion()
    return false // 空壳函数
}
