...
public class ActivityMusic extends Activity {
    ...
    // Equalizer fields
    private final Slider[] mEqualizerSlider = new Slider[EQUALIZER_MAX_BANDS];
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        // Set accessibility label for bass boost and virtualizer strength sliders.
        ...
        final int numPresets =
                ControlPanelEffect.getParameterInt(
                        mContext,
                        mCallingPackageName,
                        mAudioSession,
        ...
        for (short i = 0; i < numPresets; i++) {
            mEQPresetNames[i] =
                    ControlPanelEffect.getParameterString(
                            mContext,
                            mCallingPackageName,
                            mAudioSession,
        ...
        if ((mVirtualizerSupported)
            ...
            if (mVirtualizerSupported) {
                ...
                final Slider slider = (Slider) findViewById(R.id.vIStrengthSeekBar);
                slider.setValueFrom(OpenSLESConstants.VIRTUALIZER_MIN_STRENGTH);
                slider.setValueTo(OpenSLESConstants.VIRTUALIZER_MAX_STRENGTH);
                ...
                slider.addOnChangeListener(
                        new Slider.OnChangeListener() {
                            ...
                            public void onValueChange(
                                ...
                                if (fromUser) {
                                    ControlPanelEffect.setParameterInt(
                                            mContext,
                                            mCallingPackageName,
                                            mAudioSession,
            ...
            if (mBassBoostSupported) {
                ...
                final Slider slider = (Slider) findViewById(R.id.bBStrengthSeekBar);
                slider.setValueFrom(OpenSLESConstants.BASSBOOST_MIN_STRENGTH);
                slider.setValueTo(OpenSLESConstants.BASSBOOST_MAX_STRENGTH);
                ...
                slider.addOnSliderTouchListener(
                        new Slider.OnSliderTouchListener() {
                            @Override
                            public void onStartTrackingTouch(Slider slider) {
                                if (slider.getValue() == OpenSLESConstants.BASSBOOST_MIN_STRENGTH) {
                                    ControlPanelEffect.setParameterBoolean(
                                            mContext,
                                            mCallingPackageName,
                                            mAudioSession,
    ...
    private void reverbSpinnerInit(Spinner spinner) {
        ArrayAdapter<String> adapter =
                new ArrayAdapter<String>(
                        this, android.R.layout.simple_spinner_item, PRESETREVERBPRESETSTRINGS);
    ...
    /**
     * Updates UI (checkbox, sliders, enabled states) according to the current stored preferences.
     */
    private void updateUI() {
        final boolean isEnabled =
                ControlPanelEffect.getParameterBoolean(
                        mContext,
                        mCallingPackageName,
                        mAudioSession,
                        ControlPanelEffect.Key.global_enabled);
        ...
        if (mVirtualizerSupported) {
            Slider slider = (Slider) findViewById(R.id.vIStrengthSeekBar);
        ...
        if (mBassBoostSupported) {
            ((Slider) findViewById(R.id.bBStrengthSeekBar))
                    .setValue(
                            ControlPanelEffect.getParameterInt(
                                    mContext,
                                    mCallingPackageName,
    ...
    private void updateUIHeadset() {
        if (mToggleSwitch.isChecked()) {
            ((TextView) findViewById(R.id.vIStrengthText))
                    .setEnabled(mIsHeadsetOn || !mVirtualizerIsHeadphoneOnly);
            ((Slider) findViewById(R.id.vIStrengthSeekBar))
                    .setEnabled(mIsHeadsetOn || !mVirtualizerIsHeadphoneOnly);
            ...
            ((Slider) findViewById(R.id.bBStrengthSeekBar)).setEnabled(mIsHeadsetOn);
            findViewById(R.id.bBLayout).setEnabled(!mIsHeadsetOn);
        }
    }

    /** Initializes the equalizer elements. Set the Sliders and Spinner listeners. */
    private void equalizerBandsInit(View eqcontainer) {
        ...
        mEQPresetUserBandLevelsPrev =
                ControlPanelEffect.getParameterIntArray(
                        mContext,
                        mCallingPackageName,
                        mAudioSession,
        ...
        for (int band = 0; band < mNumberEqualizerBands; band++) {
            ...
            mEqualizerSlider[band] = (Slider) eqcontainer.findViewById(EQViewElementIds[band][1]);
            eqcontainer
                    .findViewById(EQViewElementIds[band][0])
                    .setLabelFor(EQViewElementIds[band][1]);
            mEqualizerSlider[band].setValueFrom(mEqualizerMinBandLevel);
            mEqualizerSlider[band].setValueTo(mEqualizerMaxBandLevel);

            // Set slider listener
            final int bandIdx = band; // final copy for inner class
            mEqualizerSlider[band].addOnChangeListener(
                    new Slider.OnChangeListener() {
                        @Override
                        public void onValueChange(Slider slider, float value, boolean fromUser) {
                            if (fromUser) {
                                final short level = (short) value;
                                equalizerBandUpdate(bandIdx, level);
                            }
                        }
                    });

            mEqualizerSlider[band].addOnSliderTouchListener(
                    new Slider.OnSliderTouchListener() {
                        @Override
                        public void onStartTrackingTouch(Slider slider) {
                            // get current levels
    ...
    private void equalizerUpdateDisplay() {
        ...
        for (short band = 0; band < mNumberEqualizerBands; band++) {
            ...
            mEqualizerSlider[band].setValue(level);
        }
    }
